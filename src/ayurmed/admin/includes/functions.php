...
function login()
{
    if (isset($_POST['login'])) {
        ...
        $query = "SELECT  admin_email , admin_id , admin_password FROM admin_shop WHERE admin_email= '$adminEmail' ";
        $data = query($query);
        if ($data == 0) {
...
function all_admins()
{
    $query = "SELECT admin_id ,admin_fname ,admin_lname ,admin_email  FROM admin_shop";
    $data = query($query);
    return $data;
}
function get_admin($id)
{
    $query = "SELECT admin_id ,admin_fname ,admin_lname ,admin_email  FROM admin_shop WHERE admin_id=$id";
    $data = query($query);
    return $data;
}
function edit_admin($id)
{
    if (isset($_POST['admin_update'])) {
        ...
        if ($check == 0) {
            $query = "UPDATE admin_shop SET admin_email='$email' ,admin_fname='$fname' ,admin_lname='$lname' ,admin_password='$password'  WHERE admin_id= '$id'";
            single_query($query);
...
{
    $query = "SELECT admin_email FROM admin_shop WHERE admin_email='$email'";
    $data = query($query);
    if ($data) {
        return $data;
    } else {
...
function add_admin()
{
    if (isset($_POST['add_admin'])) {
        ...
        $check = check_email_admin($email);
        if ($check == 0) {
            $query = "INSERT INTO admin_shop (admin_fname, admin_lname, admin_email, admin_password) 
...
function delete_admin()
{
    if (isset($_GET['delete'])) {
        $adminId = $_GET['delete'];
        $query = "DELETE FROM admin_shop WHERE admin_id ='$adminId'";
        $run = single_query($query);
        get_redirect("admin.php");
    }
}
function search_admin()
{
    if (isset($_GET['search_admin'])) {
        ...
        $query = "SELECT admin_id ,admin_fname ,admin_lname ,admin_email FROM admin_shop WHERE admin_email='$email'";
        $data = query($query);
        if ($data) {
            return $data;
        } else {
...
{
    $query = "SELECT admin_id FROM admin_shop where admin_id='$id'";
    $row = query($query);
    if (empty($row)) {
        return 0;
    } else {
